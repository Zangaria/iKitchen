...
export const addIngredient = async (req, res) => {
  if (
    ...
    return res
      .status(400)
      .json({ msg: 'params missing' });

  const name = req.body.name;
  const category = req.body.category;
  const info = req.body.info;
  const ingredMeasure = req.body?.ingredMeasure;
  const ingredScaleToGr =
    req.body?.ingredScaleToGr;
  ...
  if (!response?.err) {
    ...
    const uDate= new Date(getIsraelDateTime());
    const userID=req.body?.userID;
    const docID=response;
    const data={docRef,docType,uDate,userID,docID};
